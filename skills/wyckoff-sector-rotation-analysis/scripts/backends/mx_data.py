...
class MXData:
    ...
    def write_output_files(
        ...
        """Write output files. Prefer Excel when pandas/openpyxl is available, otherwise JSON."""
        ...
        if pd is not None:
            ...
            try:
                ...
                file_path = output_dir / f"mx_data_{safe_name}.json"
                file_path.write_text(json.dumps(tables, ensure_ascii=False, indent=2), encoding="utf-8")
        else:
            file_path = output_dir / f"mx_data_{safe_name}.json"
            file_path.write_text(json.dumps(tables, ensure_ascii=False, indent=2), encoding="utf-8")
        ...
        description_lines = [
            ...
            f"数据行数: {total_rows}",
            f"表数量: {len(tables)}",
            f"Sheet/表 列表: {', '.join([t['sheet_name'] for t in tables])}",
            f"导出格式: {'excel' if file_path.suffix == '.xlsx' else 'json'}",
        ]
        if condition_parts:
...
def main():
    ...
    if len(sys.argv) < 2:
        ...
        print(f"默认输出目录: skills/wyckoff-sector-rotation-analysis/scripts/backends/output/")
