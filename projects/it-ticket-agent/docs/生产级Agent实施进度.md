...
M2（context / memory）与 M4（execution / event ledger）尚未开始主体实现
```
...
- `session/service.py`、`current_agent`、`last_checkpoint_id`、`closed_at` 仍未补齐
...
- 当前 `/resume` 仅支持 approval interrupt，对 clarification / external_event 的通用恢复仍留给 A6
...
- 目前仍存在 approval facade / legacy payload / domain model 并存的过渡状态
- 还未完全消除核心链路中的 legacy 转换
...
1. `A2` 会话轮次记录落地
2. `A5` Conversation API 落地
3. `A6` 通用恢复流程接入 graph
4. `A4` Checkpoint 模型落地
5. `B1` / `B2` 上下文统一模型与 assembler
6. `B3` / `B4` 会话记忆与过程记忆
7. `C1` ~ `C4` 审批正式化收口
8. `D1` ~ `D3` 执行与事件账本
9. `S1` ~ `S4` smoke 验收补齐
...
- `A1` session 骨架已完成
- `A3` interrupt 骨架已完成
- 现在最缺的是正式的 conversation transcript / turn ledger
- 这会直接影响后续 conversation API、恢复、事件账本与上下文装配
