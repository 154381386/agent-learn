...
class SessionStoreV2:
        ...
        with self._connect() as conn:
            conn.execute(
                ...
                insert or replace into conversation_session (
                    session_id, thread_id, ticket_id, user_id, status, current_stage,
                    incident_state_json, latest_approval_id, pending_interrupt_id, last_checkpoint_id, metadata_json,
    ...
    def get_session(self, session_id: str) -> Optional[ConversationSession]:
        with self._connect() as conn:
            row = conn.execute(
                """
                select session_id, thread_id, ticket_id, user_id, status, current_stage,
                       incident_state_json, latest_approval_id, pending_interrupt_id, last_checkpoint_id, metadata_json,
                       created_at, updated_at, last_active_at
    ...
    def update_session_state(
        ...
        *,
        ...
        latest = latest_approval_id if latest_approval_id is not None else existing.latest_approval_id
        pending_interrupt = (
            pending_interrupt_id if pending_interrupt_id is not None else existing.pending_interrupt_id
        )
        latest_checkpoint = (
            last_checkpoint_id if last_checkpoint_id is not None else existing.last_checkpoint_id
        )
        with self._connect() as conn:
            conn.execute(
                """
