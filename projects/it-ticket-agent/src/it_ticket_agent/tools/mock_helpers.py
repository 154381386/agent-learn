...
def build_context(
    ...
    *,
    ...
    return {
        ...
        "service": arguments.get(target_key) or shared.get(target_key, default_target),
...
def canonical_name(value: str, aliases: dict[str, str] | None = None) -> str:
    ...
    if not normalized:
        return normalized
    alias_map = dict(COMMON_SERVICE_ALIASES)
    if aliases:
        alias_map.update(aliases)
    return alias_map.get(normalized.lower(), alias_map.get(normalized, normalized))
...
def _load_env_mock_scenarios() -> dict[str, str]:
    ...
    if not isinstance(payload, dict):
        return {}
    return {
        canonical_name(str(service)): str(scenario).strip()
...
def resolve_mock_scenario(
    ...
    shared_global = str(shared.get("mock_scenario") or "").strip()
    if shared_global:
...
def resolve_profile_mock(
    ...
    *,
    ...
    profile = profiles.get(target_name, {}).get(str(scenario).strip(), {})
